...
describe("Logic Gates", () => {
    test("AND Gate", () => {
        const table = BitsUtil.generateSignals(2).map((m) =>
            LogicGates.AND(m[0], m[1])
        );
    ...
    test("OR Gate", () => {
        const table = BitsUtil.generateSignals(2).map((m) =>
            LogicGates.OR(m[0], m[1])
        );
        expect(table).toStrictEqual([0, 1, 1, 1]);
    });

    test("NOT Gate", () => {
        const table = BitsUtil.generateSignals(1).map((m) =>
            LogicGates.NOT(m[0])
        );
